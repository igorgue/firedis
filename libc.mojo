...
fn fcntl[*T: AnyType](fildes: c_int, cmd: c_int, *args: *T) -> c_int:
    ...
    Args:
        ...
        args: Arguments for the command, args: Arguments for the command.
    Returns: Upon successful completion,0 shall be returned;
...
fn close(fildes: c_int) -> c_int:
    ...
    Args:
        fildes: A File Descriptor to close.
    Returns: Upon successful completion, 0 shall be returned; otherwise, -1
...
fn fseek(stream: Pointer[FILE], offset: c_long, whence: c_int) -> c_int:
    ...
    Args:
        ...
        whence: A pointer to a buffer to store the length of the address of the accepted socket.
    Returns: A File Descriptor or -1 in case of failure
    """
    return external_call[
...
fn getline(
    ...
    Args:
        ...
        n: A pointer to a buffer to store the length of the address of the accepted socket.
        stream: A pointer to a stream.
    Returns: Size of the lines read.
    """
    return external_call[
        "getline",
        c_ssize_t,  # FnName, RetType
...
fn feof(stream: Pointer[FILE]) -> c_int:
    ...
    Args:
        stream: A pointer to a stream.
    Returns: 1 if the end-of-file indicator associated with the stream is set, else 0.
    """
...
fn ferror(stream: Pointer[FILE]) -> c_int:
    ...
    Args:
        stream: A pointer to a stream.
    Returns: 1 if the error indicator associated with the stream is set, else 0.
    """
...
fn __test_socket_server__() raises:
    ...
    _ = printf(
        "server: started at %s : %d with fd %d – waiting for connections...\n",
        ip_addr,
        port,
        sockfd,
    )
